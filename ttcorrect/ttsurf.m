...
% check options
if(~isstring_local(wave) || ~any(strcmpi(wave,validwave)))
    error('seizmo:ttsurf:badInput',...
        'WAVETYPE must be either ''Rayleigh'' or ''Love''!');
elseif(~isstring_local(speed) || ~any(strcmpi(speed,validspeed)))
    error('seizmo:ttsurf:badInput',...
        'VELOTYPE must be either ''Group'' or ''Phase''!');
elseif(~isstring_local(model) || ~any(strcmpi(model,validmodel)))
    error('seizmo:ttsurf:badInput',...
